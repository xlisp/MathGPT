...
| 90（1-epoch 跑）| 9.25% | **43.0%** ← 峰值 | RL 早期：在保持多样性的同时提升准确率，最健康的阶段 |
| 232（1-epoch 跑末步）| 4.75% | 28.5% | **Mode collapse**：模型收敛到少数解题模板，多样性崩塌，连 Pass@1 都跌了 |
| 480（另一次 3-epoch 跑）| **14.75%** ← 峰值 | 33.5% | Pass@1 回升到峰值，但 Pass@16 永久受损 |

> ⚠️ **重要勘误**（2026-06 取证发现）：上表数据存在实验污染——`runs/tb_logs/` 显示 RL 实际跑了**两次**（一次 3-epoch、一次 1-epoch v2 配置），checkpoint 写入了同一目录互相覆盖，评估表实为两次训练的拼接（step 30~232 来自 1-epoch 跑，240 及之后来自 3-epoch 跑）。因此「U 型反弹」是拼接伪影，step 90 与 step 480 两个峰值来自不同训练、不可比。详见 `docs/NEXT_STEPS_IMPROVEMENT.md` 第〇节。

不过核心教训依然成立：在干净的单次 1-epoch 链（step 30→232）内，Pass@16 也从 43% 跌至 28.5%——**mode collapse 是没有 KL 正则的 REINFORCE 的经典病症**。RL 的本质是把 Pass@16 的潜力压缩成 Pass@1 的确定性，压缩过度就是灾难。这个实验还附赠了一课：**实验管理（checkpoint 归属可审计）和算法改进同样重要**。
