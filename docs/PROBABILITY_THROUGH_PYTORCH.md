...
- **第三部分·代码**（核心七层，全程用本项目源码）：
  ...
  3. top-k：砍掉概率分布的长尾
  4. 交叉熵：损失函数其实就是"惊讶程度"
  5. 条件概率 → 贝叶斯网络 → 因果推断：Transformer 站在阶梯的哪一级
  6. 策略梯度：REINFORCE 用"期望"教模型自我提升
- **第四部分·落地**：debug Transformer / 预测微信泊松流量 / 生活中的概率思维
...
| 贝叶斯网络 | 注意力的依赖结构 | 画清"谁直接影响谁"，别把链条上的远亲当直接原因 |
...
本文 10 张图均由 matplotlib 生成，逻辑与本项目代码一致
（历史时间线 / 中心极限定理 / softmax / top-k / 交叉熵 / 条件概率链 / 贝叶斯网络与因果阶梯 / 优势函数 / 泊松 / 熵）。
配套脚本见 `docs/images/prob/make_figs.py` 与 `make_figs2.py`，核心数学就是上文每段贴出的那几行。
