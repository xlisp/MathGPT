...
# ── Python 3.12 + PyTorch 2.3 兼容：torch.compile Dynamo 不支持 ──
# 检测 torch.compile 是否可用，若不可用则替换为 no-op 装饰器
...
if not _is_compile_supported():
    _orig_compile = torch.compile
    def _noop_compile(*args, **kwargs):
        # 情况 A: @torch.compile 或 model = torch.compile(model, ...)
        #          第一个位置参数是 callable → 直接返回
        if args and callable(args[0]):
            return args[0]
        # 情况 B: @torch.compile(dynamic=False, ...)  → 返回恒等装饰器
        return lambda fn: fn
...
def adamw_step_fused(
    ...
    """
    # 兼容非编译模式：0-D CPU 标量张量需移到与参数相同的设备
    dev = p.device
    step_t  = step_t.to(dev)
    lr_t    = lr_t.to(dev)
    beta1_t = beta1_t.to(dev)
    beta2_t = beta2_t.to(dev)
    eps_t   = eps_t.to(dev)
    wd_t    = wd_t.to(dev)
...
def muon_step_fused(
    ...
    """
    # 兼容非编译模式：将 0-D CPU 标量张量移到与参数相同的设备
    dev = stacked_grads.device
    momentum_t = momentum_t.to(dev)
    lr_t       = lr_t.to(dev)
    wd_t       = wd_t.to(dev)
    beta2_t    = beta2_t.to(dev)
