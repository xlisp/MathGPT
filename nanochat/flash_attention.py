...
# SDPA helpers
# =============================================================================
def _expand_kv_for_gqa(q, k, v):
    """若 q 的 head 数多于 k/v，将 k/v 重复展开以匹配（GQA 兼容）。"""
    h_q, h_kv = q.size(1), k.size(1)
    if h_q != h_kv:
        assert h_q % h_kv == 0, f"H_q ({h_q}) 必须能整除 H_kv ({h_kv})"
        n = h_q // h_kv
        k = k.repeat_interleave(n, dim=1)
        v = v.repeat_interleave(n, dim=1)
    return k, v

# 检测 SDPA 是否原生支持 enable_gqa（PyTorch 2.5+）
def _sdpa_supports_enable_gqa():
    try:
...
_SDPA_HAS_GQA = _sdpa_supports_enable_gqa()
...
def _sdpa_attention(q, k, v, window_size, enable_gqa):
    ...
    # 若 SDPA 不支持 enable_gqa，手动展开 k/v
    if enable_gqa and not _SDPA_HAS_GQA:
        k, v = _expand_kv_for_gqa(q, k, v)
        enable_gqa = False  # 展开后无需再传 enable_gqa
